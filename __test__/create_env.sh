...
(cd $env_dir/next-test; npm install @reflct/react)
...
(cd $env_dir/vite-test; npm install @reflct/react)
...
cat $tmp_dir/.env | sed 's/SCENE_ID/VITE_SCENE_ID/g' | sed 's/REFLCT_API_KEY/VITE_API_KEY/g' > $env_dir/vite-test/.env
